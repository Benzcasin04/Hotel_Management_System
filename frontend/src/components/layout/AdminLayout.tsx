...
import {
  Building2, LayoutDashboard, BedDouble, CalendarCheck, CreditCard,
  Users, Settings, LogOut, ChevronLeft, Menu, Mail, Bell, Check, Trash2, X,
} from 'lucide-react';
...
const AdminLayout = () => {
  ...
  const handleNotificationClick = (notification: Notification) => {
    ...
    switch (notification.type) {
      case 'booking':
      case 'payment':         navigate('/admin/bookings'); break;
      case 'guest_note':      navigate('/admin/bookings'); break;
  ...
  if (!user || (user.role as string) !== 'admin') return <Navigate to="/login" replace />;

  const links = [
    { to: '/admin',          label: 'Dashboard', icon: LayoutDashboard, gradient: 'linear-gradient(135deg,#6366f1,#4f46e5)' },
    { to: '/admin/rooms',    label: 'Rooms',     icon: BedDouble,       gradient: 'linear-gradient(135deg,#10b981,#059669)' },
    { to: '/admin/bookings', label: 'Bookings',  icon: CalendarCheck,   gradient: 'linear-gradient(135deg,#f59e0b,#d97706)' },
    { to: '/admin/payments', label: 'Payments',  icon: CreditCard,      gradient: 'linear-gradient(135deg,#8b5cf6,#7c3aed)' },
    { to: '/admin/users',    label: 'Users',     icon: Users,           gradient: 'linear-gradient(135deg,#ec4899,#db2777)' },
    { to: '/admin/messages', label: 'Messages',  icon: Mail,            gradient: 'linear-gradient(135deg,#0ea5e9,#0284c7)' },
    { to: '/admin/settings', label: 'Settings',  icon: Settings,        gradient: 'linear-gradient(135deg,#a3a3a3,#737373)' },
  ];

  const isActive = (path: string) => location.pathname === path;

  // On mobile the sidebar is always "expanded" visually when open
  ...
  const SidebarContent = () => (
    <>
      ...
      <div style={{
          ...
          <Link to="/" style={{ display: 'flex', alignItems: 'center', gap: 10, textDecoration: 'none' }}>
            ...
            <div>
              <span style={{
                ...
                color: TEXT_LIGHT, letterSpacing: '0.02em',
        ...
        {/* On mobile: close (X) button; on desktop: chevron collapse toggle */}
        {isMobile ? (
          <button
      ...
      {sidebarExpanded && (
        <div style={{
          ...
          <div style={{ overflow: 'hidden' }}>
            ...
            <p style={{ fontSize: 9, color: GOLD, textTransform: 'uppercase', letterSpacing: '0.15em' }}>Admin</p>
          </div>
        </div>
      )}
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: isMobile ? 8 : 14, flexShrink: 0 }}>
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <button
                  style={{
                    position: 'relative', background: 'rgba(196,160,90,0.08)',
                    border: '1px solid rgba(196,160,90,0.2)',
                    cursor: 'pointer', color: MUTED_DARK,
                    display: 'flex', alignItems: 'center',
                    padding: '7px 8px', borderRadius: 8,
              ...
              <DropdownMenuContent
                align="end"
                // Narrower on mobile so it doesn't overflow screen
                style={{
