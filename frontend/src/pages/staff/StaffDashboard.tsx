...
const StaffDashboard = () => {
  ...
  // Main content: stacked on mobile/tablet → sidebar layout on desktop
  const mainGrid = isDesktop ? '1fr 320px' : '1fr';
    ...
    <div className="animate-fade-in" style={{ fontFamily: 'Georgia, serif', color: TEXT }}>
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
            <div style={{
              ...
              fontSize: isMobile ? 16 : 20, fontWeight: 700, color: '#1a1612',
            ...
            <div>
              ...
              <p style={{ fontSize: 12, color: 'rgba(247,243,238,0.48)', marginTop: 3 }}>
                Welcome back,{' '}
                <span style={{ color: 'rgba(247,243,238,0.85)', fontWeight: 600 }}>{user?.name}</span>
              </p>
            </div>
          </div>

          {/* Occupancy badge — full width row on mobile */}
          <div style={{
            padding: isMobile ? '10px 16px' : '12px 20px',
            borderRadius: 10,
            background: 'rgba(196,160,90,0.12)',
            border: '1px solid rgba(196,160,90,0.25)',
            // On mobile: align left under the avatar row
            ...
            <p style={{ fontSize: 9, textTransform: 'uppercase', letterSpacing: '0.22em', color: 'rgba(196,160,90,0.7)', marginBottom: isMobile ? 0 : 4 }}>
              Occupancy
            </p>
            <p style={{ fontFamily: 'Georgia, serif', fontSize: isMobile ? '1.4rem' : '1.8rem', fontStyle: 'italic', fontWeight: 300, color: GOLD, lineHeight: 1 }}>
              {occupancyPct}%
            </p>
            <p style={{ fontSize: 9, color: 'rgba(247,243,238,0.35)', marginTop: isMobile ? 0 : 2 }}>
        ...
        {statMeta.map(meta => (
          ...
          >
            ...
            <div style={{
              ...
              borderRadius: 9, background: meta.gradient,
              display: 'flex', alignItems: 'center', justifyContent: 'center',
              marginBottom: isMobile ? 10 : 14, marginTop: 4,
              boxShadow: `0 3px 10px ${meta.glow}`,
            }}>
              <meta.icon size={isMobile ? 13 : 16} color="#fff" />
            </div>
            <div style={{
              fontFamily: 'Georgia, serif',
            ...
            <div style={{ fontSize: isMobile ? 8 : 9, textTransform: 'uppercase', letterSpacing: '0.12em', color: GOLD, fontWeight: 700 }}>
              {meta.label}
            </div>
          </div>
        ))}
      ...
      <div style={{ marginBottom: isMobile ? 16 : 20 }}>
        ...
        <div style={{
          ...
          // On mobile each button stretches to fill the row
          ...
          {quickActions.map(action => (
            <Link
              ...
              style={{
                ...
                padding: isMobile ? '12px 16px' : '11px 22px',
                borderRadius: 10,
      ...
      {/* ── Main Content Grid ── */}
      <div style={{ display: 'grid', gridTemplateColumns: mainGrid, gap: 16 }}>
        ...
        <div style={{
          ...
          // On tablet, limit height slightly
          minWidth: 0,
        }}>
                  ...
                  >
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ fontSize: 12, fontWeight: 600, color: TEXT, marginBottom: 2, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                        {room?.name
                          ? <>{room.name} <span style={{ color: MUTED, fontWeight: 400 }}>— {booking.id.slice(0, 10)}…</span></>
                          : `Booking ${booking.id.slice(0, 14)}…`
                        }
                      </div>
                      <div style={{ display: 'flex', alignItems: 'center', flexWrap: 'wrap', gap: '2px 6px', fontSize: 10, color: MUTED }}>
                    ...
                    {/* Hide pill on very small screens to avoid overflow */}
                    {!isMobile && <StatusPill status={booking.status} />}
                    {isMobile && (
                      <span style={{
                        width: 10, height: 10, borderRadius: '50%',
                        background: scfg.dot, flexShrink: 0,
                      }} />
                    )}
        ...
        {/* Right column: Today's Activity + Room Conditions + Staff note */}
        {/* On mobile/tablet these stack after the bookings panel */}
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            ...
            {todayBookings.length === 0 ? (
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                ...
                {todayBookings.length > 3 && (
                  <p style={{ fontSize: 10, color: MUTED, textAlign: 'center', marginTop: 4 }}>
          ...
          <div style={{
            ...
            {/* On mobile, use a 2-col grid for the condition rows */}
            <div style={{
              ...
              {[
                ...
                <div key={row.label} style={{
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    ...
                    <span style={{ fontSize: isMobile ? 11 : 12, fontWeight: 600, color: TEXT }}>{row.label}</span>
                  </div>
                  <span style={{
                    fontFamily: 'Georgia, serif', fontSize: '1rem',
                    fontStyle: 'italic', fontWeight: 300, color: row.color,
                  }}>
                    {row.count}
                  </span>
                </div>
